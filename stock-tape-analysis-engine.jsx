...
async function ldRecent() { try { const r = await window.storage.get("stae7:recent"); return r ? JSON.parse(r.value) : []; } catch (e) { return []; } }
async function svRecent(r) { try { await window.storage.set("stae7:recent", JSON.stringify(r)); } catch (e) { } }
...
const Pnl = ({ title, color = P.am, children }) => <div style={{ background: P.pn, border: "1px solid " + P.bd, overflow: "hidden" }}><div style={{ padding: "6px 10px", background: P.cd, borderBottom: "1px solid " + P.bd, fontSize: 11, fontWeight: 700, letterSpacing: 1, color, fontFamily: FN }}>{"▸ " + title}</div><div style={{ padding: 10 }}>{children}</div></div>;
const Pill = ({ active, onClick, children }) => <button onClick={onClick} style={{ padding: "7px 12px", border: "none", cursor: "pointer", fontFamily: FN, fontSize: 10, fontWeight: 700, letterSpacing: .8, background: active ? P.am + "18" : "transparent", color: active ? P.am : P.t3, borderBottom: active ? "2px solid " + P.am : "2px solid transparent" }}>{children}</button>;

function Chrt({ bars, vwap, H, L, cumDs }) {
  if (!bars || !bars.length) return null;
  const W = 680, Ht = 130, pd = { t: 6, b: 18, l: 44, r: 8 }, cW = W - pd.l - pd.r, cH = Ht - pd.t - pd.b;
  ...
  return <svg viewBox={"0 0 " + W + " " + Ht} style={{ width: "100%", height: "auto", display: "block" }}>
    {[.25, .5, .75].map(p => { const py = pd.t + p * cH; return <g key={p}><line x1={pd.l} y1={py} x2={W - pd.r} y2={py} stroke={P.bd} strokeWidth={.5} /><text x={pd.l - 4} y={py + 3} fill={P.t3} fontSize={9} fontFamily={FN} textAnchor="end">{n2(H - p * (H - L))}</text></g>; })}
    ...
    {[9.5, 10, 11, 12, 13, 14, 15, 16].map(h => { const s = h * 3600; if (s < minS || s > maxS) return null; return <text key={h} x={x(s)} y={Ht - 4} fill={P.t3} fontSize={9} fontFamily={FN} textAnchor="middle">{h === 9.5 ? "9:30" : Math.floor(h) + ":00"}</text>; })}
  </svg>;
}
  ...
  return (
    <div style={{ fontFamily: FN, background: P.bg, color: P.t1, minHeight: "100vh", fontSize: 11 }}>
      ...
      <div style={{ background: "linear-gradient(180deg,#0a1224,#060d19)", borderBottom: "1px solid " + P.bd, padding: "0 16px", display: "flex", alignItems: "center", height: 36, gap: 12 }}>
        ...
        {S && <span style={{ color: S.score >= 0 ? P.g : P.r, fontSize: 13, fontWeight: 700 }}>{S.score >= 0 ? "+" : ""}{S.score}</span>}
      </div>
      ...
      {S && tab !== "upload" && tab !== "hist" && <div style={{ display: "flex", gap: 1, padding: "4px 12px", background: P.bg, borderBottom: "1px solid " + P.bd, flexWrap: "wrap" }}>
        {recs.map((r, i) => <button key={i} onClick={() => setSelD(r.date)} style={{ padding: "2px 10px", border: "1px solid " + (selD === r.date ? P.am : P.bd), background: selD === r.date ? P.am + "15" : "transparent", color: selD === r.date ? P.am : P.t3, fontFamily: FN, fontSize: 10, cursor: "pointer" }}>{r.date}</button>)}
      </div>}
      ...
      <div style={{ padding: 8, maxWidth: 1440, margin: "0 auto", display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        {tab === "dash" && S && <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ background: "linear-gradient(135deg," + P.pn + "," + P.cd + ")", border: "1px solid " + P.bd, padding: 14 }}>
          ...
          <Pnl title={"SCENARIOS: #1 " + S.scenarios[0].l + " " + S.scenarios[0].p + "%"} color={S.scenarios[0].c}><PBar sc={S.scenarios} /><div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 4 }}>{S.scenarios.map((s, i) => <div key={i} style={{ background: s.c + "0a", border: "1px solid " + s.c + "22", padding: 6, textAlign: "center" }}><div style={{ fontSize: 10, color: s.c, fontWeight: 700 }}>#{i + 1} {s.l}</div><div style={{ fontSize: 22, fontWeight: 700, color: s.c }}>{s.p}%</div></div>)}</div></Pnl>
          <div style={{ display: "grid", gridTemplateColumns: "3fr 2fr", gap: 8 }}>
            <Pnl title="INTRADAY + CUM DELTA" color={P.cy}><Chrt bars={S.bars} vwap={S.vwap} H={S.H} L={S.L} cumDs={S.cumDs} /><div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 2, marginTop: 4 }}>{[{ l: "O", v: n2(S.O) }, { l: "H", v: n2(S.H), c: P.g }, { l: "L", v: n2(S.L), c: P.r }, { l: "C", v: n2(S.Cl), c: S.Cl >= S.O ? P.g : P.r }, { l: "VWAP", v: n2(S.vwap), c: P.am }, { l: "POC", v: n2(S.poc), c: P.y }, { l: "VOL", v: n2(S.TV / 1e6, 1) + "M" }].map((s, i) => <div key={i} style={{ textAlign: "center" }}><div style={{ fontSize: 9, color: P.t3 }}>{s.l}</div><div style={{ fontSize: 12, fontWeight: 700, color: s.c || P.t1 }}>{s.v}</div></div>)}</div></Pnl>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
            {[{ t: "BULL", c: P.g, items: S.sigs.filter(s => s.v > 0) }, { t: "BEAR", c: P.r, items: S.sigs.filter(s => s.v < 0) }, { t: "NEUT", c: P.y, items: S.sigs.filter(s => s.v === 0) }].map((col, ci) => <Pnl key={ci} title={col.t + " (" + col.items.length + ")"} color={col.c}>{col.items.length === 0 ? <div style={{ color: P.t4 }}>None</div> : col.items.sort((a, b) => Math.abs(b.v) - Math.abs(a.v)).map((s, i) => <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "2px 0", borderBottom: "1px solid " + P.bd, fontSize: 10 }}><span style={{ color: P.t2 }}>{s.t}</span><Tag c={col.c}>{s.v > 0 ? "+" : ""}{s.v}</Tag></div>)}</Pnl>)}
          </div>
        </div>}

        {tab === "summary" && S && (() => {
          const sections = buildSummary(S);
          return <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {/* Header banner */}
            <div style={{ background: "linear-gradient(135deg," + P.pn + "," + P.cd + ")", border: "1px solid " + P.bd, padding: "14px 18px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
              <div>
                <div style={{ fontSize: 10, color: P.t3, letterSpacing: 2, marginBottom: 4 }}>EXECUTIVE SUMMARY — {S.ticker} · {S.date}</div>
                <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                  <Tag c={S.biasC}>{S.bias}</Tag>
                  <Tag c={P.t3}>{S.dayType}</Tag>
                  <span style={{ fontSize: 22, fontWeight: 700, color: S.biasC, lineHeight: 1 }}>{S.score >= 0 ? "+" : ""}{S.score}</span>
                  <span style={{ fontSize: 11, color: P.t2 }}>composite score</span>
                </div>
              </div>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {S.scenarios.map((sc, i) => <div key={i} style={{ background: sc.c + "12", border: "1px solid " + sc.c + "30", padding: "4px 10px", textAlign: "center" }}>
                  <div style={{ fontSize: 9, color: sc.c, fontWeight: 700, letterSpacing: 1 }}>{sc.l}</div>
                  <div style={{ fontSize: 16, fontWeight: 700, color: sc.c }}>{sc.p}%</div>
                </div>)}
              </div>
            </div>
            {/* Two-column sections */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {sections.map((sec, si) => (
                <div key={si} style={{ background: P.pn, border: "1px solid " + P.bd, overflow: "hidden" }}>
                  <div style={{ padding: "7px 12px", background: P.cd, borderBottom: "1px solid " + P.bd, display: "flex", alignItems: "center", gap: 8 }}>
                    <div style={{ width: 3, height: 14, background: sec.color, borderRadius: 1 }} />
                    <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1.2, color: sec.color, fontFamily: FN }}>{sec.title}</span>
                  </div>
                  <div style={{ padding: "10px 14px", display: "flex", flexDirection: "column", gap: 6 }}>
                    {sec.bullets.map((b, bi) => (
                      <div key={bi} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                        <span style={{ color: b.c, fontSize: 13, lineHeight: 1, marginTop: 1, flexShrink: 0 }}>▸</span>
                        <span style={{ fontSize: 11, color: P.t2, lineHeight: 1.7, fontFamily: FN }}>{b.t}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {/* Bottom disclaimer */}
            <div style={{ background: P.pn, border: "1px solid " + P.bd, padding: "8px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: 10, color: P.t3, fontFamily: FN }}>STAE EXECUTIVE SUMMARY — AUTO-GENERATED FROM TAPE ANALYSIS</span>
              <span style={{ fontSize: 10, color: P.t3, fontFamily: FN }}>NOT FINANCIAL ADVICE — FOR INFORMATIONAL USE ONLY</span>
            </div>
          </div>;
        })()}

        {tab === "price" && S && <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <Pnl title="PRICE STRUCTURE" color={P.cy}><Chrt bars={S.bars} vwap={S.vwap} H={S.H} L={S.L} cumDs={S.cumDs} /><div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 6, marginTop: 8 }}>{[{ l: "PREV", v: S.prevClose ? "$" + n2(S.prevClose) : "--" }, { l: "OPEN", v: "$" + n2(S.O), s: "Gap " + n2(S.gap, 1) + "%" }, { l: "HIGH", v: "$" + n2(S.H), c: P.g, s: S.hTs.substring(0, 8) }, { l: "LOW", v: "$" + n2(S.L), c: P.r, s: S.lTs.substring(0, 8) }, { l: "CLOSE", v: "$" + n2(S.Cl), c: S.Cl >= S.O ? P.g : P.r, s: n2(S.dChg, 1) + "%" }, { l: "VWAP", v: "$" + n2(S.vwap, 4), c: P.am }, { l: "POC", v: "$" + n2(S.poc), c: P.y }, { l: "CL LOC", v: n2(S.cL * 100, 1) + "%" }].map((s, i) => <div key={i}><div style={{ fontSize: 9, color: P.t3, letterSpacing: 1.5 }}>{s.l}</div><div style={{ fontSize: 13, fontWeight: 700, color: s.c || P.t1 }}>{s.v}</div>{s.s && <div style={{ fontSize: 10, color: P.t3 }}>{s.s}</div>}</div>)}</div></Pnl>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <Pnl title="VOLUME PROFILE (12)" color={P.y}>{S.topN.map((nd, i) => { const pctW = S.topN[0] ? nd.vol / S.topN[0].vol * 100 : 0; return <div key={i} style={{ display: "grid", gridTemplateColumns: "52px 1fr 60px", gap: 4, alignItems: "center", padding: "3px 0", borderBottom: "1px solid " + P.bd }}><span style={{ fontSize: 11, fontWeight: nd.price === S.poc ? 700 : 400, color: nd.price === S.poc ? P.am : P.t1 }}>{n2(nd.price)}</span><div style={{ height: 3, background: P.bd }}><div style={{ width: pctW + "%", height: "100%", background: nd.price >= S.Cl ? P.g : P.r }} /></div><span style={{ fontSize: 10, color: P.t3, textAlign: "right" }}>{n2(nd.vol / 1e6)}M{nd.price === S.poc ? " \u25C6" : ""}</span></div>; })}</Pnl>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <Pnl title="RESISTANCE" color={P.r}>{S.ress.map((r, i) => <div key={i} style={{ padding: "4px 0", borderBottom: "1px solid " + P.bd }}><div style={{ display: "flex", gap: 4, alignItems: "center" }}><span style={{ fontSize: 13, fontWeight: 700, color: P.r }}>{n2(r.price)}</span><Tag c={P.r}>{r.src}</Tag></div><div style={{ fontSize: 10, color: P.t3 }}>{r.ev}</div></div>)}</Pnl>
              <Pnl title="SUPPORT" color={P.g}>{S.sups.map((s, i) => <div key={i} style={{ padding: "4px 0", borderBottom: "1px solid " + P.bd }}><div style={{ display: "flex", gap: 4, alignItems: "center" }}><span style={{ fontSize: 13, fontWeight: 700, color: P.g }}>{n2(s.price)}</span><Tag c={P.g}>{s.src}</Tag></div><div style={{ fontSize: 10, color: P.t3 }}>{s.ev}</div></div>)}</Pnl>
            </div>
          </div>
        </div>}
        ...
        {tab === "flow" && S && <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            ...
            <Pnl title="PHASES" color={P.cy}>{[["OPEN 9:30-10", S.phases.od], ["MIDM 10-11:30", S.phases.mm], ["MIDD 11:30-14", S.phases.md], ["PWRH 14:30-16", S.phases.ph], ["LAST 15:30-16", S.phases.l30]].map(([n, p], i) => <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "3px 0", borderBottom: "1px solid " + P.bd }}><span style={{ color: P.t2, fontSize: 10 }}>{n}</span><div style={{ display: "flex", gap: 6 }}><span style={{ color: P.t3, fontSize: 10 }}>{n2(p.v / 1e6, 1)}M</span><Tag c={p.chg > 0.5 ? P.g : p.chg < -0.5 ? P.r : P.y}>{p.chg >= 0 ? "+" : ""}{n2(p.chg, 1)}%</Tag></div></div>)}</Pnl>
          </div>
          {S.topVA.length > 0 && <Pnl title="VOLUME ACCELERATION" color={P.cy}><div style={{ display: "grid", gridTemplateColumns: "repeat(" + Math.min(5, S.topVA.length) + ",1fr)", gap: 4 }}>{S.topVA.map((a, i) => <div key={i} style={{ background: P.cd, padding: 6, textAlign: "center" }}><div style={{ fontSize: 10, color: P.t3 }}>{a.time}</div><div style={{ fontSize: 13, fontWeight: 700, color: P.g }}>+{n2(a.pct, 0)}%</div><div style={{ fontSize: 10, color: P.t3 }}>{n2(a.vol / 1e3, 0)}K</div></div>)}</div></Pnl>}
        </div>}

        {tab === "inst" && S && <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <Pnl title="TOP 10 PRINTS" color={P.mg}>{S.top10.map((t, i) => <div key={i} style={{ display: "grid", gridTemplateColumns: "22px 85px 65px 65px 48px 52px", gap: 4, padding: "4px 0", borderBottom: "1px solid " + P.bd, alignItems: "center" }}><span style={{ color: P.t3, fontSize: 10 }}>{i + 1}</span><span style={{ fontWeight: 700, fontSize: 11 }}>{t.sz.toLocaleString()}</span><span style={{ color: t.p >= S.vwap ? P.g : P.r, fontWeight: 700, fontSize: 11 }}>${n2(t.p)}</span><span style={{ color: P.t3, fontSize: 10 }}>{t.ts.substring(0, 8)}</span><Tag c={P.t3}>{t.ph}</Tag><Tag c={t.it === "DIST" ? P.r : t.it === "ACCUM" ? P.g : P.cy}>{t.it}</Tag></div>)}</Pnl>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
        ...
        {tab === "micro" && S && <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          ...
          <Pnl title="SIZE DISTRIBUTION" color={P.cy}>{[["<100", S.sBk.ol], ["100", S.sBk.r1], ["100-499", S.sBk.s15], ["500-999", S.sBk.s59], ["1K-5K", S.sBk.s1k], ["5K-10K", S.sBk.s5k], ["10K-50K", S.sBk.s10k], ["50K+", S.sBk.s50k]].map(([l, d], i) => <div key={i} style={{ display: "grid", gridTemplateColumns: "58px 1fr 38px", gap: 3, padding: "2px 0", borderBottom: "1px solid " + P.bd, alignItems: "center", fontSize: 10 }}><span style={{ color: P.t3 }}>{l}</span><div style={{ height: 3, background: P.bd }}><div style={{ width: (S.TV > 0 ? d.v / S.TV * 100 : 0) + "%", height: "100%", background: P.cy }} /></div><span style={{ color: P.t3, textAlign: "right", fontSize: 10 }}>{S.TV > 0 ? n2(d.v / S.TV * 100, 1) : 0}%</span></div>)}<Rw l="ODD LOTS" v={n2(S.oddPct, 1) + "%"} /></Pnl>
          <Pnl title="ICEBERGS | ALGOS | IMPACT" color={P.mg}><Rw l="ICEBERGS" v={S.iceCount} />{S.topIce.slice(0, 3).map((ic, i) => <Rw key={i} l={"5x" + ic.sz.toLocaleString() + " @" + n2(ic.p)} v={n2(ic.sp, 1) + "s"} />)}<div style={{ fontSize: 10, color: P.t3, marginTop: 6 }}>ALGO SIGNATURES</div>{Object.entries(S.algoS).map(([sz, d], i) => <Rw key={i} l={sz + "-SHR"} v={d.c.toLocaleString() + " (" + n2(d.p, 1) + "%)"} />)}<Rw l="IMPACT" v={"$" + n2(S.avgImp, 4) + "/M"} /><Rw l="VPIN" v={n2(S.vpin * 100) + "%"} c={S.vpin > 0.3 ? P.r : P.t2} /></Pnl>
        </div>}
        ...
        {tab === "levels" && S && <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <Pnl title="NEXT-DAY ACTION LEVELS" color={P.am}>{S.ndL.map((l2, i) => <div key={i} style={{ display: "grid", gridTemplateColumns: "48px 65px 1fr", gap: 6, padding: "6px 0", borderBottom: "1px solid " + P.bd, alignItems: "start" }}><Tag c={l2.c}>{l2.lb}</Tag><span style={{ fontSize: 14, fontWeight: 700, color: l2.c }}>${n2(l2.price)}</span><span style={{ fontSize: 10, color: P.t2, borderLeft: "2px solid " + l2.c + "33", paddingLeft: 8 }}>{l2.act}</span></div>)}</Pnl>
          <Pnl title="GAME PLAN" color={P.cy}><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6 }}>{[{ t: "BULLISH", c: P.g, b: "Open > $" + n2(S.Cl) + "\nReclaim VWAP\nLong above R1\nStop $" + n2(S.Cl * 0.97) }, { t: "BEARISH", c: P.r, b: "Open < $" + n2(S.Cl) + "\nReject VWAP\nShort below S1\nStop $" + n2(S.Cl * 1.03) }, { t: "CHOP/REV", c: P.y, b: "S1-R1 range\n50% size\nFade extremes\nVol confirm" }].map((card, i) => <div key={i} style={{ background: card.c + "08", border: "1px solid " + card.c + "22", padding: 8 }}><div style={{ fontSize: 10, color: card.c, fontWeight: 700, marginBottom: 4 }}>IF {card.t}</div><div style={{ fontSize: 10, color: P.t2, lineHeight: 1.8, whiteSpace: "pre-line" }}>{card.b}</div></div>)}</div></Pnl>
        </div>}
